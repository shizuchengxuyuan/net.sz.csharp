...
namespace Net.Sz.Framework.Netty
{
    ...
    public abstract class IOSession
    {
        ...
        /// 消息编码器
        /// </summary>
        internal DefaultMarshalEndian mershaEncoder = null;
        /// <summary>
        /// 获取远程终结点
        /// </summary>
        ...
        internal void SetSocket()
        {
            ...
            this.ReceiveAsync();
            this.IP = this.RemoteEndPoint.Address.ToString();
            this.Port = this.RemoteEndPoint.Port;
            if (isServer) { NettyPool.SessionHandler.ChannelActive(this); }
            else { NettyPool.ClientSessionHandler.ChannelActive(this); }
        }

        public IOSession()
        {
            mershaEncoder = (DefaultMarshalEndian)NettyPool.MarshalEndianType.Assembly.CreateInstance(NettyPool.MarshalEndianType.FullName);
        }
                ...
                if (!this.IsDispose)
                {
                    ...
                    try
                    {
                        if (log.IsDebugEnabled())
                            log.Debug("Close Tcp Socket Remote Socket LocalEndPoint：" + LocalEndPoint + " RemoteEndPoint：" + RemoteEndPoint + " : " + msg);
                        if (isServer) { NettyPool.SessionHandler.ChannelUnregistered(this, msg); }
                        else { NettyPool.ClientSessionHandler.ChannelUnregistered(this, msg); }
        ...
        public void WriteAndFlush(int msgId, byte[] buf)
        {
            ...
            {
                if (isServer) { NettyPool.SessionHandler.ExceptionCaught(this, ex); }
                else { NettyPool.ClientSessionHandler.ExceptionCaught(this, ex); }
            }
            finally
            {
                outBuf.Close();
