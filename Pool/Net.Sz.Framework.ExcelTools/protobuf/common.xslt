...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" exclude-result-prefixes="msxsl">
  ...
  <xsl:template name="PickNamespace">
    <xsl:param name="defaultNamespace"/>
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:variable name="trimmedName">
          <xsl:choose>
        <xsl:when test="substring(name,string-length(name)-5,6)='.proto'">
          <xsl:value-of select="substring(name,1,string-length(name)-6)"/>
        </xsl:when>
      <xsl:otherwise>
