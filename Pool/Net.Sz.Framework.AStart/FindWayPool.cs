...
namespace Net.Sz.Framework.AStart
{
    ...
    /// 
    /// </summary>
    public class FindWayPool
    {
        static readonly FindWayPool instance = new FindWayPool();
        public static FindWayPool GetInstance() { return instance; }
        ...
        private Point GetMinFFromOpenList(List<Point> Open_List)
        {
            ...
            {
                if (Pmin == null || Pmin.F > Open_List[i].F)
                    Pmin = Open_List[i];
            }
            return Pmin;
        }
        ...
        //判断关闭列表是否包含一个坐标的点
        private bool IsInCloseList(int x, int y, Dictionary<string, Point> Close_List)
        {
            return Close_List.ContainsKey(x + "-" + y);
        }

        //从关闭列表返回对应坐标的点
        private Point GetPointFromCloseList(int x, int y, Dictionary<string, Point> Close_List)
        {
            if (Close_List.ContainsKey(x + "-" + y))
            {
                return Close_List[x + "-" + y];
            }
            return null;
        }
        ...
        //检查当前节点附近的节点
        private void CheckP8(Point p0, byte[,] map, Point pa, Point pb, List<Point> Open_List, Dictionary<string, Point> Close_List)
        {
            //这里的循环其实就是8方向判断
        ...
        public Point FindWay(byte[,] r, int sx, int sz, int ex, int ez)
        {
                ...
                && !CheckBlocking(r, pb))
            {
                ...
                Open_List.Add(pa);
                while (!(IsInOpenList(pb.X, pb.Y, Open_List) || Open_List.Count == 0))
                {
